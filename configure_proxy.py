...
#get proxy ip
folder_path_proxy = "proxy_image"
file_path_proxy = os.path.join(folder_path_proxy, "instance_details.json")
...
if __name__ == "__main__":
    ...
    if os.path.exists(file_path_proxy):
        ...
        print(f"Worker2 Private IP: {worker2_private_ip}")

        # Access proxy informations
        proxy_info = data.get("proxy_instance", {})
        proxy_public_ip = proxy_info.get("PublicIP", "Public IP not found")
        proxy_private_ip = proxy_info.get("PrivateIP", "Private IP not found")

        print(f"proxy Public IP: {proxy_public_ip}")
        print(f"proxy Private IP: {proxy_private_ip}")
    
    else:
        print(f"Error: File {file_path_proxy} does not exist.")
        sys.exit(1)
    #################################################
        # Define paths and parameters

    tar_file_name = "proxy_image.tar.gz"
    username = "ubuntu"  # Default EC2 username
    private_key_path = "./my-key-pair.pem"  # Path to your private key
    remote_filepath = "/home/ubuntu/proxy_image.tar.gz"

 
    
    #stable for connection
    if not wait_for_ssh(proxy_public_ip, username, private_key_path):
        print(f"Failed to establish SSH connection to {proxy_public_ip}")
        

    #Create the tar.gz archive
    create_tar_gz(folder_path_proxy, tar_file_name)
    # local_path = "./sync_manager_worker/instance_details.json"
    local_path = f"./{tar_file_name}"
    # Transfer the archive to the manager
    transfer_file(proxy_public_ip , username, private_key_path, local_path , remote_filepath)

    run_docker(proxy_public_ip , username, private_key_path, tar_file_name)
