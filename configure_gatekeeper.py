...
#get proxy ip
folder_path_gatekeeper = "gatekeeper"
file_path_gatekeeper = os.path.join(folder_path_gatekeeper, "instance_details.json")
...
if __name__ == "__main__":
    ...
    if os.path.exists(file_path_gatekeeper):
        with open(file_path_gatekeeper, 'r') as file:
            data = json.load(file)

        gatekeeper_info = data.get("gatekeeper_instance", {})
        gatekeeper_public_ip = gatekeeper_info.get("PublicIP", "Public IP not found")
        gatekeeper_private_ip = gatekeeper_info.get("PrivateIP", "Private IP not found")
    
    else:
        print(f"Error: File {file_path_gatekeeper} does not exist.")
        sys.exit(1)
    #################################################
        # Define paths and parameters

    tar_file_name = "gatekeeper.tar.gz"
    username = "ubuntu"  # Default EC2 username
    private_key_path = "./my-key-pair.pem"  # Path to your private key
    remote_filepath = "/home/ubuntu/gatekeeper.tar.gz"

 
    
    #stable for connection
    if not wait_for_ssh(gatekeeper_public_ip, username, private_key_path):
        print(f"Failed to establish SSH connection to {gatekeeper_public_ip}")
        

    #Create the tar.gz archive
    create_tar_gz(folder_path_gatekeeper, tar_file_name)
    # local_path = "./sync_manager_worker/instance_details.json"
    local_path = f"./{tar_file_name}"
    # Transfer the archive to the manager
    transfer_file(gatekeeper_public_ip , username, private_key_path, local_path , remote_filepath)

    run_docker(gatekeeper_public_ip , username, private_key_path, tar_file_name)
